...
#[derive(FromPrimitive)] // compiler complains
enum Opcode {
    ADD   =  1,   
    MUL   =  2,  
    READ  =  3,    
...
#[derive(PartialEq)]       // compiler complains this doesn't exist...
...
impl Machine {
    ...
    fn write_to_memory(&mut self, parameter : usize, mode : AddressingMode, value : i64) {
        match mode {
            AddressingMode::Immediate => panic!("Can only write in Address mode"),
    ...
    fn read_from_memory(&self, parameter : usize, mode : AddressingMode) -> i64 {
        match mode {
            AddressingMode::Immediate => return self.memory[parameter],
            AddressingMode::Location =>  return self.memory[self.memory[parameter] as usize]
        }
    }
    ...
    pub fn run_to_block (&mut self) -> MachineStatus {
        loop {
            ...
            match opcode {
                ...
                Opcode::READ => {
                    if self.input.is_empty() { return MachineStatus::ReadBlocked; }
