...
impl Machine {
    ...
    pub fn tick(&mut self) -> MachineStatus {
    ...
    pub fn tick_with_default_read(&mut self, default: i64) -> MachineStatus {
        let mut status = self.tick();
        if status == MachineStatus::ReadBlocked {
            self.input.push(default);
            status = self.tick();
        }
        status
    }
    ...
    pub fn run_to_block (&mut self) -> MachineStatus {
        let mut status = self.tick();
        while status == MachineStatus::Ready {
            status = self.tick();
        }
        status
    }
...
fn test_rel_base() {
    ...
    output_tape.clear();    
    do_intcode(&numbers_2, &input_tape, &mut output_tape);
    assert_eq!(*output_tape.last().unwrap(),34915192*34915192);

    let numbers_3 =  vec![104,1125899906842624,99];
    output_tape.clear();    
    do_intcode(&numbers_3, &input_tape, &mut output_tape);
